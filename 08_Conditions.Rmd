...
The simplest way to handle conditions is to ignore them:
...
tryCatch(
  {
    ...
    # always run
    unlink(path) # Delete directory specified by path.
  }
)
```
...
`abort()` stores the whole backtrace generated by `rlang::trace_back()`
